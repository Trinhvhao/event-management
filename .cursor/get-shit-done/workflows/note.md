...
- **Global scope**: `D:/LibraryCode/event-management/.cursor/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `D:/LibraryCode/event-management/.cursor/notes/`)
...
2. Glob `D:/LibraryCode/event-management/.cursor/notes/*.md` (if directory exists) — global notes
...
Global (D:/LibraryCode/event-management/.cursor/notes/):
...
2. **No `.planning/`**: Falls back to global `D:/LibraryCode/event-management/.cursor/notes/` — works in any directory
